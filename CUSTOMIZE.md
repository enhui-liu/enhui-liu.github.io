...
└── 📂 _sass/: contains the SASS files that define the style of the website
    ...
    ├── 📄 _components.scss: reusable component styles (cards, profiles, CV)
...
This Jekyll theme implements [collections](https://jekyllrb.com/docs/collections/) to let you break up your work into categories. The theme comes with two default collections: `news` and `books`. Items from the `news` collection are automatically displayed on the home page, while items from the `books` collection are displayed on their own `bookshelf` page.
...
4. **Add a navigation link to your collection page**
  ...
  Update the navigation configuration of your page. In the frontmatter of your collection landing page (e.g., `_pages/courses.md`), add:
...
Add the following to your page's Markdown file:
...
- remove the navigation link to the blog page, if you added one
...
- remove the navigation link to the publications page, if you added one
...
- The scheduler works by moving posts from the `_scheduled/` directory to `_posts/`, it will not post to other collections such as `_news/`
